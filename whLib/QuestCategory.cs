...
namespace Wowhead
{
    ...
    public class QuestCategoryList : List<QuestCategory>
    {
        ...
        public void Load(bool forceRequest = false)
        {
            ...
            Regex myRegex = new Regex(@"mn_quests=\[(\[(?<id>\-?\d*),""(?<name>[\w '\-\():]+)""(?<sub>(,,\[(\[\-?\d+,""[\w '\-\():]+""],?)*])?)],?)*]", RegexOptions.Compiled);
        ...
        {
            Regex myRegex = new Regex(@"\[(\[(?<id>\-?\d*),""(?<name>[\w '\-\():]+)""(?<sub>(,,\[(\[\-?\d+,""[\w '\-\():]+""],?)*])?)],?)*]", RegexOptions.Compiled);
            ...
            {
                Group gId = mc[0].Groups["id"];
                Group gName = mc[0].Groups["name"];
                Group gSub = mc[0].Groups["sub"];

                for (int j = 0; j < gId.Captures.Count; j++)
                {
                    QuestCategory qcat = new QuestCategory() { Name = gName.Captures[j].Value };
                    this.Add(qcat);

                    if (!string.IsNullOrEmpty(gId.Captures[j].Value))
                        qcat.Id = int.Parse(gId.Captures[j].Value);

                    if (!string.IsNullOrEmpty(gSub.Captures[j].Value))
                    {
                        qcat.Categories = new QuestCategoryList();

                        qcat.Categories.ParseItems(gSub.Captures[j].Value);
                    }
                }
            }
        }
